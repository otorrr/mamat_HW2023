...
do
	...
	if [[($(cat link_result.txt | grep "Netanyahu" | wc -l) -eq 0) 
	&& ($(cat link_result.txt | grep "Gantz" | wc -l) -eq 0) 
	&& ($(cat link_result.txt | grep "Lapid" | wc -l) -eq 0) 
	&& ($(cat link_result.txt | grep "Ben-gvir" | wc -l) -eq 0)]]; 
	then echo ", -" >> results.csv #no accurrences, neither one of them	   
    else
